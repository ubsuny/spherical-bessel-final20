...
def root_simple(f, n, x, dx, accuracy=1.0e-6, max_steps=1000, root_debug=False):
    ...
    iterations = []
    if root_debug:        
        root_print_header("Simple Search", accuracy)
...
def root_bisection(f,n, x1, x2, accuracy=1.0e-6, max_steps=1000, root_debug=False):
...
def root_secant(f, n, x0, x1, accuracy=1.0e-6, max_steps=20,root_debug=False):
...
def root_tangent(f, fp, n, x0, accuracy=1.0e-6, max_steps=20, root_debug=False):
